...
Page({
  ...
  onFilterChange(e) {
    const isDebt = e.currentTarget.dataset.isDebt;
    this.setData({
      currentIsDebt: isDebt
    });
    this.loadOrderList(true);
  },
  ...
  loadOrderList(refresh = false) {
    ...
    if (this.data.currentIsDebt !== '') {
      params.isDebt = parseInt(this.data.currentIsDebt);
    }

    request.get('/outbound/list', params).then(function(res) {
